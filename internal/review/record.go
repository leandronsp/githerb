...
func CommentRecord(comment Comment) Record {
	return Record{kind: KindComment, comment: comment, resolution: noResolution, check: noCheck, chunk: noChunk}
}

// ResolutionRecord wraps a resolution as a log record.
func ResolutionRecord(resolution Resolution) Record {
	return Record{kind: KindResolve, comment: noComment, resolution: resolution, check: noCheck, chunk: noChunk}
}

// CheckRecord wraps a check result as a log record.
func CheckRecord(check Check) Record {
	return Record{kind: KindCheck, comment: noComment, resolution: noResolution, check: check, chunk: noChunk}
}

// ChunkRecord wraps a decision as a log record.
func ChunkRecord(chunk Chunk) Record {
	return Record{kind: KindChunk, comment: noComment, resolution: noResolution, check: noCheck, chunk: chunk}
}
...
func RationaleRecord(comment Comment) Record {
	return Record{kind: KindRationale, comment: comment, resolution: noResolution, check: noCheck, chunk: noChunk}
}
